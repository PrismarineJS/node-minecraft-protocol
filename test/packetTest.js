...
const { Client, Server } = mc;
const { utils: { getFieldInfo, getField } } = require('protodef')
...
mc.supportedVersions.forEach(function(supportedVersion,i) {
  ...
  describe('packets ' + version.minecraftVersion, function() {
    ...
    let packetInfo;
    Object.keys(protocol).filter(function(state) {return state !== 'types'}).forEach(function(state) {
      Object.keys(protocol[state]).forEach(function(direction) {
        Object.keys(protocol[state][direction].types)
          .filter(function(packetName) {return packetName !== 'packet' && packetName.startsWith('packet_')})
          .forEach(function(packetName) {
            packetInfo = protocol[state][direction].types[packetName];
