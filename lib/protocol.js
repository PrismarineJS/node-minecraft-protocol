  ...
  "login": {
    ...
    "toServer": {
      ...
      0x01: [
        ...
        { name: "countVerifyToken", type: "short" },
        { name: "verifyToken", type: "byteArray16", count: "countVerifyToken" }
      ]
    }
  },
  "play": {
...
function parsePacket(buffer, state, isServer) {
  ...
  function readPacketField(fieldInfo, packet, localCursor) {
    if ('count' in fieldInfo) {
      var newFieldInfo = {
        name: fieldInfo.name,
        type: fieldInfo.type,
        contents: fieldInfo.contents,
        condition: fieldInfo.condition
      }
      var results = {
        size: 0,
        value: new Array(packet[fieldInfo.count])
      };
      if (fieldInfo.type === "byte") {
        results.value = new Buffer(packet[fieldInfo.count]);
      }
      for (var i = 0;i < packet[fieldInfo.count];i++) {
        var readResults = readPacketField(newFieldInfo, packet, localCursor);
        if (! readResults) return null;
        if (readResults.error) return { error: readResults.error };
        results.size += readResults.size;
        localCursor += readResults.size;
        results.value[i] = readResults.value;
      }
      return results;
    } else if (fieldInfo.type === "container") {
      var results = {
        size: 0,
        value: {}
      }
      fieldInfo.contents.forEach(function (subFieldInfo) {
        var readResults = readPacketField(subFieldInfo, packet, localCursor);
        if (! readResults) return null;
        if (readResults.error) return { error: readResults.error };
        results.size += readResults.size;
        localCursor += readResults.size;
  ...
  for (i = 0; i < packetInfo.length; ++i) {
    ...
    readResults = readPacketField(fieldInfo, results, cursor);
    if (!!!readResults) {
